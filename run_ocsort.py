...
def compare_dataframes(gts, ts):
    ...
    for k, tsacc in ts.items():
        if k in gts:
            ...
            accs.append(
                mmp.utils.compare_to_groundtruth(
                    gts[k], tsacc, "iou", distth=0.5, vflag=vflag
                )
            )
            names.append(k)
            vflag.close()
...
def main(exp, args, num_gpu):
    ...
    result_folder = (
        "{}_test_results".format(args.expn)
        if args.test
        else "{}_results".format(args.expn)
    )
    ...
    *_, summary = evaluator.evaluate_ocsort(
    ...
    ts = OrderedDict(
        [
            (
                ...
                mmp.io.loadtxt(f, fmt="mot15-2D", min_confidence=-1),
            )
            for f in tsfiles
    ...
    print(
        mmp.io.render_summary(
            summary, formatters=mh.formatters, namemap=mmp.io.motchallenge_metric_names
        )
    )
